...
const RATE_OMOK_START: RateLimitConfig = {
  capacity: 5,
  refillPerSec: 5 / 60,
};
...
export async function POST() {
  ...
  if (!consumeToken(`omok-start:${me.memberId}`, RATE_OMOK_START)) {
    return NextResponse.json({ error: "rate_limited" }, { status: 429 });
  }

  // 판 시작은 방장만. 오목은 1:1이라 혼자서는 시작할 수 없다.
  ...
  const { matchId, startedAt } = startMatch({
    ...
    onEnded: async (results: OmokResult[], groupId: string) => {
      ...
      try {
        ...
        console.error("omok match result persist failed", e);
      }
    },
  });
